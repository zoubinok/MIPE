...
##To adapt to high degenerate primers. I used primer0 to genenrate all primer binding sites.
###################################################################################################################################
...
=head1 Usage
    ...
    $0 -i <input> -p <primer_file> -n <sign> -r <reference_file> -l <length_of_elongation> -k <min_percent_of_pbsite_length>
...
=head1 Parameters
    ...
    -l  [int]   The extension nt to extract primer-binding-sites. 1-5 is required.
    -k  [int]   The min percent of pbsite length to filt sequences not aligned in this part. 1 is recommanded.
...
#gelp print
die `pod2text $0` if ((!$input_fasta) or (!$input_primer) or (!$sign) or (!$input_ref_seq) or (!$length_of_elongation) or (!$min_percent_of_pbsite_length));
   ...
   foreach my $pname (@primer_names) 
   {
	 ...
	 my $pbsite_file = "$pname".'_primer_binding_sites.fa';
      ...
      if ($start_position{$pname}<$length_of_aligned_seq)
      {
		  ...
		  my $seq_start=substr($aligned_seq,0,$start_position{$pname}+$length_of_elongation-2+1);
		  $seq_start =~ s/[^ATCGUatcgu]//g;
		  $seq_start = '-' x $length_of_elongation . $seq_start; #insure it would not less than 0;
		  #my $seq_start_length=; #calculate non [ATCGUatcgu] numbers
		  my $seq_extracted_start =substr($seq_start,length($seq_start)-$length_of_elongation);

		  my $seq_end=substr($aligned_seq,$end_position{$pname}-$length_of_elongation);
		  $seq_end =~ s/[^ATCGUatcgu]//g;
		  $seq_end = $seq_end . '-' x $length_of_elongation;
		  #my $seq_extracted_end_length=length($seq_extracted_end); #calculate non [ATCGUatcgu] numbers
		  my $seq_extracted_end =substr($seq_end,0,$length_of_elongation);

		 my $seq_extracted=$seq_extracted_start . substr($aligned_seq,$start_position{$pname}+$length_of_elongation-1,$end_position{$pname}-$length_of_elongation-1-($start_position{$pname}+$length_of_elongation-1)+1) . $seq_extracted_end;
	  ...
	  {
               $seq_extracted =~ s/[\.\-]//g; #delete . - to adapt hmmalign 20180110
...
open CPRIMER2, "complete_$input_primer" || die "$!";#add 180620
 my $pbsite_file_0=0;  #add 180620

while (<CPRIMER2>)#add 180620
{
  chomp;#add 180620
  s/\s+//g;
  my @name_and_seq= split /:/,$_;
    $pname=$name_and_seq[0];
	 my $pbsite_file = "$pname".'_primer_binding_sites.fa';                            #add 180620
	 if($pname =~ /[a-zA-Z]0$/){$pbsite_file_0=$pbsite_file;}#add 180620
     else{system("cp $new_dir_name/$pbsite_file_0 $new_dir_name/$pbsite_file"); 
	     print "$new_dir_name/$pbsite_file_0 $new_dir_name/$pbsite_file\n";
	 } #add 180620
}#add 180620
...
foreach my $pname(@primer_names)
{
	...
	$template_primer =~ s/[\.\-]//g; #delete . - to adapt hmmalign 20180110
	...
	#system("/home/server/hmmer-3.1b2-linux-intel-x86_64/easel/miniapps/esl-reformat afa $pname:tmp_pbsite_candidate.sto > $pname:tmp_pbsite_candidate_normal.align"); #change PATH based on your system.
	#system("perl quhuanhang.zb.pl $pname:tmp_pbsite_candidate_normal.align $pname:tmp_pbsite_candidate_one.align");
	system("perl ./scripts/hmmreformat.pl $pname:tmp_pbsite_candidate.sto $pname:tmp_pbsite_candidate_sto.align $pname:tmp_pbsite_candidate.align");
	system("./mothur \"#filter.seqs(fasta=$pname:tmp_pbsite_candidate.align)\"");
...
#######################################
#注释掉483，484行的mothur 命令，484-490 是加入的hmm align和序列的格式处理,426行把.和-替换成空
