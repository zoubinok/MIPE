...
#
#
##Usage example: perl MIPE_get_primer_binding_sites_ssu_fulllength.pl -i input.fasta -r Ref_16S.fasta -p primer1.txt -n 88888 -l 4 -k 01 
...
while (my $line=<FILT>)
{
   ...
   foreach my $pname (@primer_names) 
   {
	  ...
	  my $pbsite_name = "$pname".'_primer_binding_sites.accno';


      open PBSITE, ">>$new_dir_name/$pbsite_file" || die "$!";
      open NOPB, ">>$new_dir_name/$no_pbsite_file" || die "$!";
      open PBSITENAME, ">>$new_dir_name/$pbsite_name" || die "$!";
...
{
    my $pbsite_name = "$pname".'_primer_binding_sites.accno';
    my $pbsite_file = "$pname".'_primer_binding_sites_fulllength.fa';

    system("mothur \"#get.seqs(fasta=ssu_merge_$sign.pick.fasta,accnos=$new_dir_name/$pbsite_name)\"");
	system("mv ssu_merge_$sign.pick.pick.fasta $new_dir_name/$pbsite_file"); 
}
...
foreach my $pname(@primer_names)
{
	...
	close TEMPLATE;
        ...
        my $pbsite_file_name_fulllength=$new_dir_name . "/$pname" . '_primer_binding_sites_fulllength.fa';
		
        print LOG "Open:$pbsite_file_name\n";
        print LOG "Open:$pbsite_file_name_fulllength\n";

	system("cp $pbsite_file_name $pname:tmp_pbsite_candidate.fa");
	system("cp $pbsite_file_name_fulllength $pname:tmp_pbsite_candidate_fulllength.fa");

	open CANDI, ">>$pname:tmp_pbsite_candidate.fa" || die "$!";
	print CANDI ">primer_sequence\n$template_primer\n";
	close CANDI;

	open CANDIF, ">>$pname:tmp_pbsite_candidate_fulllength.fa" || die "$!";
	print CANDIF ">primer_sequence\n$template_primer\n";
	close CANDIF;
	...
	system("mothur \"#align.seqs(candidate=$pname:tmp_pbsite_candidate_fulllength.fa,template=$pname:tmp_primer_template)\"");
	system("mothur \"#filter.seqs(fasta=$pname:tmp_pbsite_candidate_fulllength.align)\"");
