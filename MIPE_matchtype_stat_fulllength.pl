...
open LOG, ">>$log_file.fulllength.log";
...
while (<SUITE>)
{
   ...
   my $pbsite_file_name=$new_dir_name . "/$pname" . '_primer_binding_sites_fulllength.fa';
   ...
   close PBSITE;
        ...
        open FILT,"$pname:tmp_pbsite_candidate_fulllength.filter.fasta" || die "$!";
        open LOGP, ">$pname.fulllength.log" || die $!;

        my @names_and_pbsites=<FILT>;
	close FILT;
        print LOG "Create:$pname,match_type";
	open RES,">$new_dir_name/${pname}_fulllength_match_type" || die $!;
...
# Whether the primer-binding-site is matching is rejudged.
         ...
         for (my $i=0;$i<=$degenerate_primer{$pname};$i++)
	 {
	     my $match_type_file=$pname . $i . '_fulllength_match_type';
...
print "Step 4: Compare the match scores of each primer-binding site and pick out the best matching one-\n";
#---Step 4: Compare the match scores of each primer-binding site and pick out the best matching one------------------------------------------------------#	
foreach my $pname (@degenerate_primer_names)
{
	
	open TYPE, ">$new_dir_name/${pname}_fulllength_match_type" || die "$!";	
	print TYPE "Seq_name\tForward or reverse\tMatch_type\tPrimer-binding site\tMisMatches in last 4 nt\t".
	"Mismatches\tWhether matching\tCompleteness\tTaxonomic Information\tScore\n";  
        my $primer_name_0="$pname" . '0' ; 
        my $word_count=`wc -l $new_dir_name/${primer_name_0}_fulllength_match_type`;
        $word_count =~ /^([0-9]+)\s/;
...
# mark which non-degenerate primer
	    ...
	    {
	        my $match_type_file=$pname . $j . '_fulllength_match_type';
				print "$match_type_file-------TMPTYPE$new_dir_name/${match_type_file}_tmp\n";

                open TMPTYPE, "$new_dir_name/${match_type_file}_tmp";	
				#print "TMPTYPE$new_dir_name/${match_type_file}_tmp\n";
                while (<TMPTYPE>){
				#print "XIAHUAXIAN$_";
	    	      next unless ($. == $i);
	  	      $lines[$j] = $_ ;
...
{
   open STAT, ">$new_dir_name/$pname.fulllength.stat";
   open RES, "$new_dir_name/${pname}_fulllength_match_type";
...
#---Table 3: Most frequent mismathes----------------------------------------------------------------------------# 
    ...
    my @match_type;  
    my %match_type;
    my $word_count=`wc -l $new_dir_name/${pname}_fulllength_match_type`;
    $word_count =~ /^([0-9]+)\s/;
