...
#
#
##Database in need: v119silva.SSU.fasta, v119fangsilva.SSU.silva.tax, v119silva.SSU_PICK.fasta.
# Put them and input file, primer file, reference file into the working directory.
#
#
##Usage: perl MIPE_main_program.pl -i input.fasta -p primer1.txt -r Ref_16S.fasta -l 4 -k 01 -s 10 -t 50 -c 0 -F 0000000 -b b
...
#
#
##Output file: inputtaxonomy.silva.wang.taxonomy and directory of result_$$_input
#
####################################################################################################################################
...
system("./mothur \"#classify.seqs(fasta=$ssu_merge_fasta,reference=$ssu_template,taxonomy=$taxo_ref)\"");
...
system("cp $ssu_merge_name.silva.wang.taxonomy inputtaxonomy.silva.wang.taxonomy");  ##To get taxonomy.
...
system ("rm *.pick.fasta  *.report ssu_merge*.* *.logfile .*.swo .*.swp *8mer* *train* *sum* candi*"); ##If you want to get more files, comment out the line.
